...
#接受提交过来的用户名及密码
$username = trim($_POST['user']);//用户名
$password = trim($_POST['pwd']);//密码
...
#拿着提交过来的用户名和密码去数据库查找，看是否存在此用户名以及其密码  
 include "inc.php";
$sql="select *from dy_user where user = '$username'and psw='$password' ";
...
if($rows){
  ...
  //echo '<script>location.href="user/dyadrxz.php"</script>';
